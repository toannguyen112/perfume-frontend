...
function Alert() {
    ...
    const { isAlert, type, message } = useSelector((state: any) => state.alert.alert);

    const onClose = () => {
        dispatch(
            createAlert({ isAlert: false }));
    };

    const icon = type === TYPE.SUCCESS ? "/svg/icon-success.svg" : "/svg/icon-error.svg"
